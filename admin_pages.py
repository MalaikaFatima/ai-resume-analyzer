...
def show_vacancies():
    ...
    if vacs.empty:
        ...
        return
    # # Admin helper: fix badly formatted skills fields (commas between letters)
    # if st.button("Fix skills formatting for existing vacancies", type="secondary"):
    #     with st.spinner("Repairing vacancy skills..."):
    #         res = db.repair_vacancy_skills()
    #         st.success(f"Fixed {res.get('fixed_count',0)} vacancy(ies)")
    #         st.rerun()
...
def show_upload_resumes():
    ...
    # Show two panes: resumes added by admin (not linked to a user)
    # and resumes associated with candidate user accounts
    ...
    # Build a DataFrame of all files uploaded by candidates (include multiple files per candidate)
    cand_files = []
    if cand_rows:
        try:
        ...
        for u in cand_rows:
            ...
            # If helper unavailable or returned empty, fall back to the recorded filename
            if files_df is None or files_df.empty:
                # try to include the candidate's recorded filename from DB
            ...
            for _, fr in files_df.iterrows():
                # Find applications that reference this exact uploaded file (by full path or filename)
                vac_titles = []
                try:
                    apps = db.run_query(
                ...
                # Fallback: if no explicit resume_filename matches, check if candidate's recorded filename equals this file
                if not vac_titles:
                    try:
                        rec = db.get_candidate(cid)
    ...
    with col2:
        ...
        if st.session_state.get('pending_delete_cand'):
            ...
            if pending_id == del_cand:
                # fetch candidate record for display and file unlink
...
def show_rankings():
    ...
    with st.spinner("Computing latest AI rankings..."):
        ...
        if vacancy_id is not None:
            ...
            # For each application compute score against the vacancy applied to,
            # then keep the best-scoring vacancy per candidate (in-memory, no DB writes).
            best = {}
            ...
            # Display only percentages (no progress bars)
    ...
    # Display percentages without progress bars
    display_df = df[["rank", "name", "total_score_%", "skills_score_%"]].copy()
    display_df.columns = ["Rank", "Candidate", "Overall Match (%)", "Skills Match (%)"]
    st.dataframe(display_df, use_container_width=True, hide_index=True)
...
def show_dashboard():
    ...
    # Show unique applicants (number of distinct users who applied)
    ...
    # Panel: show applicants for a selected vacancy
